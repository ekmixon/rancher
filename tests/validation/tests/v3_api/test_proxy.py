...
RANCHER_PROXY_INTERNAL_HOSTNAME = (
    f"{PROXY_HOST_NAME}-internal.qa.rancher.space"
)

RANCHER_PROXY_HOSTNAME = f"{PROXY_HOST_NAME}.qa.rancher.space"
...
def deploy_proxy_server():
    node_name = f"{PROXY_HOST_NAME}-proxy"
    proxy_node = AmazonWebServices().create_node(node_name)

    # Copy SSH Key to proxy and local dir and give it proper permissions
    write_key_command = f"cat <<EOT >> {proxy_node.ssh_key_name}.pem\n{proxy_node.ssh_key}\nEOT"

    proxy_node.execute_command(write_key_command)
    local_write_key_command = f"mkdir -p {SSH_KEY_DIR} && cat <<EOT >> {SSH_KEY_DIR}/{proxy_node.ssh_key_name}.pem\n{proxy_node.ssh_key}\nEOT"

    run_command(local_write_key_command, log_out=False)

    set_key_permissions_command = f"chmod 400 {proxy_node.ssh_key_name}.pem"
    proxy_node.execute_command(set_key_permissions_command)
    local_set_key_permissions_command = (
        f"chmod 400 {SSH_KEY_DIR}/{proxy_node.ssh_key_name}.pem"
    )
    ...
    copy_cfg_command = f'scp -q -i {SSH_KEY_DIR}/{proxy_node.ssh_key_name}.pem -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null {RESOURCE_DIR}/squid/squid.conf {AWS_USER}@{proxy_node.host_name}:~/squid/squid.conf'

    run_command(copy_cfg_command, log_out=True)

    squid_cmd = f"sudo docker run -d -v /home/ubuntu/squid/squid.conf:/etc/squid/squid.conf -p {RANCHER_PROXY_PORT}:3128 wernight/squid"


    proxy_node.execute_command(squid_cmd)

    print(
        f"Proxy Server Details:\nNAME: {node_name}\nHOST NAME: {proxy_node.host_name}\nINSTANCE ID: {proxy_node.provider_node_id}\n"
    )
...
def prepare_airgap_proxy_node(bastion_node, number_of_nodes):
    node_name = f"{PROXY_HOST_NAME}-agproxy"
    ...
    for num, ag_node in enumerate(ag_nodes):
        ...
        proxy_url = f"{bastion_node.host_name}:{RANCHER_PROXY_PORT}"
        proxy_info = f'[Service]\nEnvironment=\"HTTP_PROXY={proxy_url}\" \"HTTPS_PROXY={proxy_url}\" \"NO_PROXY=localhost,127.0.0.1,0.0.0.0,10.0.0.0/8,cattle-system.svc\"'


        bastion_node.execute_command(f'echo "{proxy_info}" > http-proxy.conf')

        ag_node_create_dir = f'ssh -i "{bastion_node.ssh_key_name}.pem" -o StrictHostKeyChecking=no {AWS_USER}@{ag_node.private_ip_address} "sudo mkdir -p /etc/systemd/system/docker.service.d"'

        bastion_node.execute_command(ag_node_create_dir)

        copy_conf_cmd = f'scp -q -i "{bastion_node.ssh_key_name}".pem -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ~/http-proxy.conf {AWS_USER}@{ag_node.private_ip_address}:~/'

        bastion_node.execute_command(copy_conf_cmd)

        ag_node_mv_conf = f'ssh -i "{bastion_node.ssh_key_name}.pem" -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null {AWS_USER}@{ag_node.private_ip_address} "sudo mv http-proxy.conf /etc/systemd/system/docker.service.d/ && sudo systemctl daemon-reload && sudo systemctl restart docker"'

        bastion_node.execute_command(ag_node_mv_conf)

        print(
            f"Airgapped Proxy Instance Details:\nNAME: {node_name}-{num}\nPRIVATE IP: {ag_node.private_ip_address}\n"
        )
...
def deploy_proxy_rancher(bastion_node):
    ...
    deploy_rancher_command = f'sudo docker run -d --privileged --restart=unless-stopped -p 80:80 -p 443:443 -e HTTP_PROXY={proxy_url} -e HTTPS_PROXY={proxy_url} -e NO_PROXY="localhost,127.0.0.1,0.0.0.0,10.0.0.0/8,cattle-system.svc" rancher/rancher:{RANCHER_SERVER_VERSION} --trace'
    ...
    deploy_result = run_command_on_proxy_node(bastion_node, ag_node,
                                              ...
                                              log_out=True)
    assert (
        f"Downloaded newer image for rancher/rancher:{RANCHER_SERVER_VERSION}"
        in deploy_result[1]
    )
...
def create_nlb_and_add_targets(aws_nodes):
    ...
    lb = AmazonWebServices().create_network_lb(name=f"{PROXY_HOST_NAME}-nlb")
    ...
    internal_lb = AmazonWebServices().create_network_lb(
        name=f"{PROXY_HOST_NAME}-internal-nlb", scheme='internal'
    )
    ...
    tg80 = AmazonWebServices().create_ha_target_group(
        80, f"{PROXY_HOST_NAME}-tg-80"
    )

    tg443 = AmazonWebServices().create_ha_target_group(
        443, f"{PROXY_HOST_NAME}-tg-443"
    )
    ...
    internal_tg80 = AmazonWebServices().create_ha_target_group(
        80, f"{PROXY_HOST_NAME}-internal-tg-80"
    )

    internal_tg443 = AmazonWebServices().create_ha_target_group(
        443, f"{PROXY_HOST_NAME}-internal-tg-443"
    )
...
def test_deploy_proxied_rancher():
    ...
    print(
        f"\nConnect to bastion node with:\nssh -i {proxy_node.ssh_key_name}.pem {AWS_USER}@{proxy_node.host_name}\nConnect to rancher node by connecting to bastion, then run:\nssh -i {proxy_node.ssh_key_name}.pem {AWS_USER}@{proxy_rancher_node.private_ip_address}\n\nOpen the Rancher UI with: https://{public_dns}\n"
    )
