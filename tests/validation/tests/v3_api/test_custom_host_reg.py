...
def test_add_custom_host():
    ...
    for aws_node in aws_nodes:
        additional_options = (
            f" --address {aws_node.public_ip_address}" + " --internal-address "
...
def test_deploy_rancher_server():
    ...
    if AUTH_PROVIDER:
        enable_url = (
            (f"{RANCHER_SERVER_URL}/v3/{AUTH_PROVIDER}" + "Configs/")
            + AUTH_PROVIDER.lower()
            + "?action=testAndApply"
        )
        ...
        auth_user_login_url = (
            f"{RANCHER_SERVER_URL}/v3-public/{AUTH_PROVIDER}Providers/"
            + AUTH_PROVIDER.lower()
        ) + "?action=login"
