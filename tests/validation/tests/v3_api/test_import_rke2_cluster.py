...
RANCHER_RKE2_KUBECONFIG_PATH = f"{DATA_SUBDIR}/rke2_kubeconfig.yaml"
...
def test_create_rancherd_multiple_control_cluster():
    ...
    rke2_clusterfilepath = create_rke2_multiple_control_cluster(cluster_type, \
                                                                cluster_version)
    fqdn_file = f"/tmp/{RANCHER_HOSTNAME_PREFIX}_fixed_reg_addr"
    with open(fqdn_file, 'r') as f:
        fqdn = f.read()
        fqdn = fqdn.strip()
    print("RANCHERD URL\nhttps://{0}:8443\n".format(fqdn), flush=True)
    ip_file = f"/tmp/{RANCHER_HOSTNAME_PREFIX}_master_ip"
    ...
    cmd = (
        (
            f"ssh -o StrictHostKeyChecking=no -i {keyPath} {RANCHER_AWS_USER}"
            + "@"
        )
        + ip
...
def create_rke2_multiple_control_cluster(cluster_type, cluster_version):
    ...
    rke2_clusterfilepath = f"{DATA_SUBDIR}/{rke2_kubeconfig_file}"

    tf_dir = f"{DATA_SUBDIR}/terraform/rke2/master"
    keyPath = os.path.abspath('.') + '/.ssh/' + AWS_SSH_KEY_NAME
    os.chmod(keyPath, 0o400)
    ...
    if int(RANCHER_RKE2_NO_OF_WORKER_NODES) > 0:
        tf_dir = f"{DATA_SUBDIR}/terraform/rke2/worker"
        tf = Terraform(working_dir=tf_dir,
                       variables={'region': RANCHER_REGION,
                                  'vpc_id': RANCHER_VPC_ID,
                                  'subnets': RANCHER_SUBNETS,
    ...
    print(tf.apply("--auto-approve"))
    print("\n\n")
    cmd = (f"cp /tmp/{RANCHER_HOSTNAME_PREFIX}_kubeconfig " + rke2_clusterfilepath)
    os.system(cmd)
    ...
    cmd = f"kubectl get nodes --kubeconfig={rke2_clusterfilepath}"
    print(run_command(cmd))
    cmd = f"kubectl get pods -A --kubeconfig={rke2_clusterfilepath}"
    print(run_command(cmd))
    print("\n\n")
...
def create_rancher_cluster(client, rke2_clusterfilepath):
    clustername = CLUSTER_NAME or random_test_name("testcustom-rke2")
    cluster = client.create_cluster(name=clustername)
    cluster_token = create_custom_host_registration_token(client, cluster)
    command = cluster_token.insecureCommand
    finalimportcommand = f"{command} --kubeconfig {rke2_clusterfilepath}"
...
def check_cluster_status(kubeconfig):
    ...
    try:
        while nodeNotReady and (retries < 10):
            cmd = f"kubectl get nodes --no-headers -A --kubeconfig={kubeconfig}"
        ...
        while podsNotReady and (retries < 10):
            cmd = f"kubectl get pods --no-headers -A --kubeconfig={kubeconfig}"
